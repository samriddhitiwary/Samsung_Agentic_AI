"""Selective high-context Jina Code 1.5B GGUF reranking for AppsRetrieval.

This experiment keeps the verified Jina Q8 first-stage retrieval fixed and
reranks only the top 5 candidates for the same lowest-margin 25% of queries
using Jina Code Embeddings 1.5B at max sequence length 1024.
"""
...
from src.retrieval.jina_code import DOCUMENT_INSTRUCTION, QUERY_INSTRUCTION, corpus_hash, row_text
...
OUTPUT_RUN = "jina_code_1.5b_selective_1024"
...
def enrich_embedding_cache_metadata(
    *,
    ...
    """Add experiment metadata to the resumable embedding-cache sidecar."""
    meta_path = embeddings_path.with_suffix(".meta.json")
...
def run_selective_pass(
    *,
    ...
    if query_embeddings.shape[1] != EXPECTED_EMBEDDING_DIMENSION:
        raise ValueError(
            f"Expected query embedding dimension {EXPECTED_EMBEDDING_DIMENSION}, "
            f"got {query_embeddings.shape[1]}"
        )
    if doc_embeddings.shape[1] != EXPECTED_EMBEDDING_DIMENSION:
        raise ValueError(
            f"Expected doc embedding dimension {EXPECTED_EMBEDDING_DIMENSION}, "
            f"got {doc_embeddings.shape[1]}"
        )
    enrich_embedding_cache_metadata(
        embeddings_path=query_cache_path,
        selected_query_ids=selected_query_ids,
        corpus_fingerprint=corpus_fingerprint,
    ...
    enrich_embedding_cache_metadata(
        embeddings_path=doc_cache_path,
        selected_query_ids=selected_query_ids,
        corpus_fingerprint=corpus_fingerprint,
...
def main() -> int:
    ...
    cache_dir = env_path(
        "JCR_JINA_CODE_1_5B_SELECTIVE_1024_CACHE_DIR",
    ...
    if not args.skip_smoke:
        smoke_query_ids = selected_query_ids[:10]
        ...
        smoke_rankings, smoke_metadata = run_selective_pass(
            ...
            cache_prefix="smoke10",
            corpus_fingerprint=corpus_fingerprint,
        )
        smoke_runtime = time.perf_counter() - smoke_start
        smoke_payload = {
            "model_loaded": True,
            "embedding_dimension": smoke_metadata["embedding_dimension"],
            "expected_embedding_dimension": EXPECTED_EMBEDDING_DIMENSION,
            "embedding_dimension_matches_expected": smoke_metadata["embedding_dimension"]
            == EXPECTED_EMBEDDING_DIMENSION,
            "finite_embeddings": smoke_metadata["finite_embeddings"],
            "finite_scores": all(
            ...
            "projected_full_runtime_hours": smoke_runtime
            / len(smoke_query_ids)
    ...
    metadata = {
        ...
        "gguf_file": GGUF_FILE,
        "quantization": QUANTIZATION,
        "embedding_dimension": rerank_metadata["embedding_dimension"],
        "expected_embedding_dimension": EXPECTED_EMBEDDING_DIMENSION,
        "max_sequence_length": MAX_LENGTH,
